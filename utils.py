...
def denoise_with_rnnoise(
    audio_base64, content_type, padding_duration=0.1, time_stretch_factor=0.75
):
    try:
        ...
        try:
            output, _ = (
                ...
                .output(
                    "pipe:",
                    format="wav",
                    af=f"{filter_chain_str},arnndn=m={model_path}",
                )
                .run(input=input_audio, capture_stdout=True, capture_stderr=True)
            )
        except ffmpeg.Error as e:
            raise RuntimeError(
                f"Error during noise reduction with FFmpeg: {e.stderr.decode()}"
            )
...
def get_error_arrays(alignments, reference, hypothesis):
    ...
    for chunk in alignments[0]:
        if chunk.type == "insert":
            ...
            substitution.append(
                {"removed": hypothesis[hyposlice], "replaced": reference[refslice]}
            )
...
def identify_missing_tokens(orig_text, resp_text):
    ...
    missing_flatList = set(
        phoneme for sublist in missing_phoneme_list for phoneme in sublist
    )
    construct_flatList = set(
        phoneme for sublist in construct_phoneme_list for phoneme in sublist
    )
