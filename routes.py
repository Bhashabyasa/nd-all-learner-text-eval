...
@router.post(
    "/getTextMatrices",
    response_model=ErrorArraysResponse,
    summary="Compute Text Matrices",
    description="Computes WER, CER, insertion, deletion, substitution, confidence char list, missing char list, construct text",
    responses={
        400: {
        ...
        500: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "detail": "Unexpected error: Error processing characters: <error_message>"
                    }
                }
            },
        },
...
async def compute_errors(data: TextData):
    try:
        ...
        if not data.reference:
            raise HTTPException(
                status_code=400, detail="Reference text must be provided."
            )
        ...
        if language not in allowed_languages:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported language: {language}. Supported languages are: {', '.join(allowed_languages)}",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error processing characters: {str(e)}"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error computing WER: {str(e)}"
            )
        ...
        if language == "en":
            try:
                confidence_char_list, missing_char_list, construct_text = processLP(
                    reference, hypothesis
                )
            except Exception as e:
                logger.error(f"Error processing LP: {str(e)}")
                raise HTTPException(
                    status_code=500, detail=f"Error processing LP: {str(e)}"
                )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error extracting error arrays: {str(e)}"
            )
...
@router.post(
    ...
    summary="Get Phonemes",
    description="Converts text into phonemes.",
    responses={
        400: {
            "description": "Bad Request",
            "content": {
        ...
        500: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "detail": "Unexpected error: Error getting phonemes: <error_message>"
                    }
                }
            },
        },
...
async def audio_processing(data: audioData):
    try:
        ...
        if not data.base64_string:
            raise HTTPException(
                status_code=400, detail="Base64 string of audio must be provided."
            )
        if not data.contentType:
            raise HTTPException(
                status_code=400, detail="Content type must be specified."
            )
        ...
        if data.enablePauseCount:
            try:
                ...
                if pause_count is None:
                    ...
                    raise HTTPException(
                        status_code=500, detail="Error during pause count detection"
                    )
            except Exception as e:
                logger.error(f"Error during pause count detection: {str(e)}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Error during pause count detection: {str(e)}",
                )

        if data.enableDenoiser:
            try:
                denoised_audio_base64 = denoise_with_rnnoise(
                    audio_data, data.contentType
                )
                if denoised_audio_base64 is None:
                    logger.error("Error during audio denoising")
                    raise HTTPException(
                        status_code=500, detail="Error during audio denoising"
                    )
            except ValueError as e:
                logger.error(f"Value error in denoise_with_rnnoise: {str(e)}")
                raise HTTPException(
                    status_code=400,
                    detail=f"Value error in denoise_with_rnnoise: {str(e)}",
                )
            except RuntimeError as e:
                logger.error(f"Runtime error in denoise_with_rnnoise: {str(e)}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Runtime error in denoise_with_rnnoise: {str(e)}",
                )
            except Exception as e:
                logger.error(f"Unexpected error in denoise_with_rnnoise: {str(e)}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Unexpected error in denoise_with_rnnoise: {str(e)}",
                )
